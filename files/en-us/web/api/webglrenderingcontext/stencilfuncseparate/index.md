...
- `func`
  ...
  - : A {{domxref("GLenum")}} specifying the test function. The default function is
...
- `ref`
  - : A {{domxref("GLint")}} specifying the reference value for the stencil test. This
    value is clamped to the range 0 to 2^n - 1 where n is the number of bitplanes
    in the stencil buffer. The default value is 0.
- `mask`
  - : A {{domxref("GLuint")}} specifying a bit-wise mask that is used to AND the reference
